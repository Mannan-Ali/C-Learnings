...
void Athead(Node *&tail, Node *&head, int data)
{
    if (head == NULL)
    {
        ...
        head = temp;
        //this means the head points to what temp is pointing to not that head points to temp as it is a pointer
        tail = temp;
        return;
    }
    Node *temp = new Node(data);
    (*temp).next = head;
    //temp is a pointer and *temp is an object
    // temp->next =head;   // (*temp).next = head;
    //he -> operator is used to dereference a pointer and access a member of the object that the pointer is pointing to, all in one step
    head = temp;
}
